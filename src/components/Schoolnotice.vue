...
/*  mounted () {
    ...
    this.$http.get(
      ...
      .then(function (response) {
        ...
        that.tableData = that.allData.slice(0, 8);
  ...
  methods:{
    handleCurrentChange(val) {  //当前页
      console.log(val);
      this.tableData = this.allData.slice((val - 1) * 8, 8 * val);
  ...
  methods: {
    ...
    getCurrentData(currentPage = 1) {
      ...
      console.log(that);
      this.$http.get(
        that.$interface + 'getArticlePages?categoryId=2'+'&pageNum='+currentPage+'&pageSize=15'
      )
        .then(function (response) {
          if (response.data.status === 1) {
            that.newsList = [];
